...
_HS_ONLY = re.compile(
    r"\b(ami score|pillar (score|rating)|kpi|index score|HS assessment|"
...
def question_needs_web_search(question: str, rag_context: object) -> bool:
    ...
    - HS scores / KPIs / pillar ratings → RAG only
    - current conflict/risk/humanitarian or source-needed facts → search
    - empty RAG for a non-HS question → search
    """
    q = question or ""
    context_text = _normalize_rag_context(rag_context)

    if _HS_ONLY.search(q) and not _CURRENT_INTEL.search(q):
        return False
