"""
    ...
    Depends on:
        ...
        prompt_templates.HSPromptTemplates   — all prompt text
...
#  HSPromptTemplates so service context stays visible)                       #
# --------------------------------------------------------------------------- #
...
class HSResearchService:
    ...
    async def research_and_score_question(
        ...
        try:
            ...
            pillar_context = HSPillarPrompts.get_pillar_context(pillarID, pillars)

            system_prompt = HSPromptTemplates.question_system_prompt(pillar_context)
    ...
    async def research_and_score_country(
        ...
        """Produce a cross-pillar country-level HornScope assessment."""
        try:
            year = year or datetime.now().year
            pillars = await db_repository.get_active_pillars_map()
            pillar_names = HSPillarPrompts.get_all_pillar_names(pillars)
            pillar_list_str = "\n".join(
                f"{k}. {v}" for k, v in pillar_names.items()
            )
            system_prompt = HSPromptTemplates.country_system_prompt(
    ...
    async def immediate_situation(
        ...
        try:
            ...
            if not documentContext or len(documentContext) < 100:
                ...
                system_prompt = HSPromptTemplates.country_situation_awareness_system_prompt(
                    pillar_list_str
                )
            else:
...
ami_ai_research_service = HSResearchService()
